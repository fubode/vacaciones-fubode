...
<html>
    ...
    <div id="areaImpresion">
        ...
        <c:choose>
            <c:when test="${codigo_entidad==0}">
                <label><strong>ENTIDADES: </strong>TODOS</label><br>
            </c:when>    
            <c:otherwise>
                <label><strong>CODIGO ENTIDAD: </strong>${codigo_entidad}</label><br>
                <label><strong>NOMBRE ENTIDAD: </strong>${nombre_entidad}</label><br>
            </c:otherwise>
        </c:choose>

        <c:choose>
            <c:when test="${codigo_cargo==0}">
                <label><strong>CARGO: </strong>TODOS</label><br>
            </c:when>    
            <c:otherwise>
                <label><strong>CODIGO CARGO: </strong>${codigo_cargo}</label><br>
                <label><strong>NOMBRE CARGO: </strong>${nombre_cargo}</label><br>
            </c:otherwise>
        </c:choose>
            ...
            <table id="tabla_reporte" class="table container-fluid table-hover table-striped table-bordered">
                <thead>
                    <tr>
                        ...
                        <th>ESTADO</th>
                        <th>RESPONSABLE</th>
                            <c:if test="${codigo_cargo==0}">
                            <th>CARGO</th>
                            </c:if>
                            <c:if test="${codigo_entidad==0}">
                            <th>ENTIDAD</th>
                            </c:if>
                    </tr>
                </thead>
